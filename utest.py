...
def control(env, save_gif=False):
    ...
    #rs = [] # for testing optimal action
    #estimate_v = env.unwrapped.test_v(discount)
    #estimate_v = env.get_v(discount)
    while not done:
        env.render()
    ...
    #assert np.isclose(discount_cumsum(rs)[0], estimate_v)
    if save_gif:
...
def record_trajs(env, filename, index=None):
    ...
    #train_combos = list(product(range(10), range(10)))#[(0, 0), (0, 1), (0, 5)]
    #test_combos = train_combos
    storage = Storage('env')
    #env = PORGBEnv(PickGridWorld(map_names, num_obj_types=5, train_combos=train_combos, test_combos=test_combos, window=4, seed=0))
...
def test_trajs(env, filename, index=None):
    ...
    if isinstance(env.unwrapped, PickGridWorld):
        #np.testing.assert_equal(origin_records['get_qs'], records['get_q']) # old records
        np.testing.assert_equal(origin_records['get_q'], records['get_q'])
        #np.testing.assert_equal([record['output'][0][0] for record in origin_records['step']], [record['output'][0] for record in records['step']]) # old
    ...
    #np.testing.assert_equal(origin_records['step'], records['step']) # this!
...
class TestPickEnv(unittest.TestCase):
    ...
    @ipdb_on_exception
    def control(self):
        #env = get_pick_env(task_length=1, num_obj_types=26)
        env = PORGBEnv(
            PickGridWorld(
                map_names=['map0'],
                num_obj_types=3,
